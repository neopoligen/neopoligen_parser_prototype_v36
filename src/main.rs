...
"#;
    match parse(content) {
        Ok(ast) => {
            // dbg!(ast);
            let out = output(&ast);
            let _ = fs::write("output.html", out);
            ()
        }
        Err(e) => {
            dbg!(e);
...
fn basic_section_start<'a>(source: &'a str, mut inside: Vec<&'a str>) -> IResult<&'a str, Node, ErrorTree<&'a str>> {
    let category = "basic";
    ...
    let (source, _) = tuple((space0, newline)).context("").parse(source)?;
    let (source, _) = tuple((space0, newline)).context("").parse(source)?;
    let (source, mut children) = many0(alt((|src| start_or_full_section(src, inside.clone()), basic_block)))
        .context("")
        .parse(source)?;
...
fn list_section_full(source: &str) -> IResult<&str, Node, ErrorTree<&str>> {
    ...
    let (source, _) = tuple((space0, newline)).context("").parse(source)?;
    let (source, _) = tuple((space0, newline)).context("").parse(source)?;
    let (source, children) = many0(list_item_full).context("").parse(source)?;
    Ok((
        source,
        Node::Wrapper {
...
fn list_section_start<'a>(source: &'a str, mut inside: Vec<&'a str>) -> IResult<&'a str, Node, ErrorTree<&'a str>> {
    let category = "list";
    inside.push(category);
    let (source, _) = tag("-- ").context("").parse(source)?;
    let (source, r#type) = list_section_tag.context("").parse(source)?;
    ...
    let (source, _) = tuple((space0, newline)).context("").parse(source)?;
    let (source, _) = tuple((space0, newline)).context("").parse(source)?;
    let (source, mut children) = many0(alt((list_item_full, |src| start_or_full_section(src, inside.clone()))))
        .context("")
        .parse(source)?;
...
fn start_or_full_section<'a>(source: &'a str, inside: Vec<&'a str>) -> IResult<&'a str, Node, ErrorTree<&'a str>> {
