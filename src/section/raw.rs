...
pub fn raw_section_full<'a>(source: &'a str, sections: &'a Sections, spans: &'a Vec<String>) -> IResult<&'a str, Node, ErrorTree<&'a str>> {
    let (source, _) = tag("-- ").context("").parse(source)?;
...
pub fn raw_section_start<'a>(source: &'a str, sections: &'a Sections, spans: &'a Vec<String>) -> IResult<&'a str, Node, ErrorTree<&'a str>> {
    let (source, _) = tag("-- ").context("").parse(source)?;
    let (source, r#type) = raw_section_tag.context("").parse(source)?;
    ...
    let (source, _) = multispace0.context("").parse(source)?;
    let (source, end_section) = raw_section_end(source, r#type)?;
    Ok((
        source,
        Node::Raw {
...
pub fn raw_section_tag<'a>(source: &'a str) -> IResult<&'a str, &'a str, ErrorTree<&'a str>> {
